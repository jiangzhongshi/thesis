\section{Local Operations}\ref{app:local-ops}
\cite[Fig. 11]{chp:shell} introduce a set of \emph{valid} local operations to modify the shell, including edge split, edge collapse, edge flip and vertex smoothing. 
...
\ref[Theorem 3.7]{chp:shell} outlines invariant conditions, which maintains the shell projection to be bijective. 
...
% (e.g., smoothing will ``move'' some edges from $\overline f_1$ to $\overline f_2$
% \DZ{it should not be "e.g.", unless you describe what exactly you are doing elsewhere; or refer to appendix. Moving edges from one set to another as a result of smoothing is unclear} 
% collapse will merge all edges in a unique feature)